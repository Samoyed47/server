...
{
	string NickName, PassWord, num;
	char Buffer[48];	
	strcpy(Buffer, buf.c_str());
	sscanf(Buffer, "%s|%s", &num, &NickName, &PassWord);

	D.SelectData(1, "\0");
	int MaxAccount = D.User1[D.User1.size() - 1].Account;
...
string UserManage::LogSuccess(string buf)//buf:账号|密码; return:9#账号#昵称#头像#好友账号1|好友账号2|#群号1|群号2|
{
	...
	MsgRecord.clear();
	for (int i = 0; i < D.Cluster1.size(); i++)
	{
		string s;
		char Buffer[1024];	
	...
	for (int i = 0; i < MsgRecord.size(); i++)//MsgRecord[i]:Time1,Acount1,Msg1|Time2,Account2,Msg2|Time3,Account3,Msg3
	{
		...
		{
			string Time, Acc, Msg;
			sscanf(AMsg, "%s,%s,%s", &Time, &Acc, &Msg);
	...
	string SQL = "CMember LIKE %" + Acc + "% AND COwner = 1";
	D.SelectData(2, SQL);
...
string UserManage::Receive(string buf)//buf:群号或好友账号|用户账号|发送时间|消息内容; return:10#Done(表示收到消息且已发出）
{
	...
	sscanf(M, "%s'", &MsgRecord);//MsgRecord:'Time1,Acount1,Msg1|Time2,Account2,Msg2|Time3,Account3,Msg3
...
string UserManage::AddFriend1(string buf)//buf:用户账号|好友账号|备注信息;该函数：发送好友请求；return : 7#sended（已发送）/You are already friends(已经是好友）
{
	...
	if (!D.Cluster1.empty())
	{
		return "7" + '#' + string("You are already friends");
	}
	...
	D.SelectData(2, "\0");
	int MaxCNum = D.Cluster1[D.Cluster1.size() - 1].CNum;
